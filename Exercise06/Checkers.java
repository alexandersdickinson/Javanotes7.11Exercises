...
public class Checkers extends JPanel {
	...
	private JButton saveButton;//Button that can be used to save the game.
	
	private JButton loadButton;//Button that can be used to load the game.

    private JLabel message;  // Label for displaying messages to the user.
	
	private JFileChooser fileDialog; //Allows user to pick files for saving and loading.
	
	private JOptionPane errorMessage; //Displays error messages.
    ...
    public Checkers() {
		...
		saveButton.setBounds(210, 90, 120, 30);
		loadButton.setBounds(210, 120, 120, 30);
    ...
    private class Board extends JPanel implements ActionListener, MouseListener {
		...
		/**
			This function reads an xml file of the user's choosing and assigns appropriate values to instance variables of Board object.
		*/
		private void loadGame(){
			
			File loadFile;
			int option;
			option = fileDialog.showOpenDialog(this);
			if(option != JFileChooser.APPROVE_OPTION)
				return;
			loadFile = fileDialog.getSelectedFile();
			if(!loadFile.getName().endsWith(".xml")){
				errorMessage.showMessageDialog(this, "File needs to be in .xml format.");
				return;
			}
			...
			try{
                ...
                xmlDoc = docReader.parse(loadFile);
				Element root = xmlDoc.getDocumentElement();
				NodeList stateData = root.getChildNodes();
				if(stateData.item(1) instanceof Element){
					System.out.println("stateData");//debug
					Element currentPlayerData = (Element)stateData.item(1);
					currentPlayer = Integer.parseInt(currentPlayerData.getAttribute("currentPlayer"));
					selectedRow = Integer.parseInt(currentPlayerData.getAttribute("selectedRow"));
					selectedCol = Integer.parseInt(currentPlayerData.getAttribute("selectedCol"));
				}
				//pieces on the board
				int k = 0;
				for(int i = 2; i < stateData.getLength(); i++){//i represents child nodes of the root. k represents index of columns
					
					if(stateData.item(i) instanceof Element){
						Element row = (Element)stateData.item(i);
						
						if(row.getTagName().equals("row")){
							System.out.println("row");//debug
							NodeList cols = row.getChildNodes();
						
							for(int j = 0; j < cols.getLength(); j ++){//One square on the board ([row][col]).
							
								int l = 0;
								if(cols.item(i) instanceof Element){
									Element col = (Element)cols.item(i);
									if(col.getTagName().equals("col")){
										System.out.println("col");//debug
										board.board[k][l] = Integer.parseInt(col.getAttribute("state"));
										if(l < 7)
											l++;
									}
								}
				...
				errorMessage.showMessageDialog(this, "Error trying to read file: " + e.getMessage());
				e.printStackTrace();
			}
		...
		/**
			This subroutine saves game data to an xml file, boardData.xml.
		*/
		void saveGame(){
			
			File saveFile;
			int option;
			option = fileDialog.showSaveDialog(this);
			if(option != JFileChooser.APPROVE_OPTION)
				return;
			...
			try{
				...
				output.println("<checkers version = \"1.0\">");
				output.println("  <playerData currentPlayer = \'" + currentPlayer + "\' selectedRow = \'"
								+ selectedRow + "\' selectedCol = \'" + selectedCol + "\' />");//Save currentPlayer, selectedRow and Col.
				
				for(int[] row : board.board){
					
					output.println("  <row>");
					for(int col : row){
						output.println("    <col state = \'" + col + "\'/>");
					}
					output.println("  </row>");
					
				}
				
				output.println("</checkers>");
				output.flush();
				output.close();
				
			}
			catch(Exception e){
				errorMessage.showMessageDialog(this, "An error occurred trying to save data: " + e.getMessage());
				e.printStackTrace();
			}
